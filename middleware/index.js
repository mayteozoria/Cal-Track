...
const comparePassword = async (storedPassowrd, password) => {
  let passwordMatch = await bcrypt.compare(password, storedPassowrd)
...
const verifyToken = (req, res, next) => {
  ...
  try {
    ...
    console.log(error)
    res.status(401).send({ status: 'Error', msg: 'Verify Toek Error!' })
  }
}
